...
const BUILTINS = new Set(`
  addcslashes array array_combine array_filter array_key_exists array_keys array_map array_values
  apache_request_headers basename bin2hex date die echo empty error_log exit explode file file_exists
  file_get_contents filter_var floatval function_exists getenv header http_response_code implode
  in_array intval is_array is_numeric is_string json_decode json_encode max microtime min number_format
  parse_url password_hash password_verify preg_match preg_replace print random_bytes round setcookie
  sort sprintf str_replace strlen strpos strtolower strtoupper strtotime substr time trim ucfirst ucwords uniqid
