...
beforeAll(() => {
  // jsdom does not implement File.prototype.arrayBuffer() (or Blob.arrayBuffer()),
  // but the app's Excel/CSV parser calls file.arrayBuffer(). Polyfill it with
  // FileReader, which jsdom fully supports.
...
async function buildXlsxFile(): Promise<File> {
  const rows = [
  ...
  const blob = await writeXlsxFile([headerRow, ...dataRows], { sheet: 'Stock Import' }).toBlob();
  return new File([blob], 'stock_import.xlsx', {
...
describe('Smart Stock Import crash (#310)', () => {
  it('opens the import modal and parses an .xlsx without crashing', async () => {
    ...
    // Upload a real .xlsx through the file input.
    const fileInput = document.querySelector('input[type="file"]') as HTMLInputElement;
    expect(fileInput).toBeTruthy();
    fireEvent.change(fileInput, { target: { files: [await buildXlsxFile()] } });
